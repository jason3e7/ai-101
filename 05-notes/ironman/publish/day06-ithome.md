...
> 開頭「你是資料整理助手」看起來像角色扮演, 但那是**脈絡**格的最短寫法, 是在講「這是整理類任務」. 跟下面會講到「角色扮演沒用」不衝突, 那條講的是吹履歷式人設 (「你是 10 年經驗的資深工程師」), 而且限客觀題; 這裡是指定任務類型, 屬於下面那條「仍然有用」的例外.
...
當你要的格式很難用文字描述時, 直接給一到三個範例, 比寫五百字說明有效得多. 原因 [Day 05](https://ithelp.ithome.com.tw/articles/10413765) 已經講過: **任務座標是從你的話裡算出來的, 範例是你操作那個座標最直接的介面**. 給模式, 比給形容詞準.

放到上面那個範本上, 就是在 `<task>` 前面多插一個 `<example>` 區塊, 貼一份「輸入 → 期望輸出」給它看.

**二、明確的輸出格式與長度.**
「用 JSON」「三點, 每點不超過 30 字」這類可驗證的約束, 它照做的機率高很多. 反過來, 「詳細一點」這種模糊詞幾乎沒有效果.
...
[Day 05](https://ithelp.ithome.com.tw/articles/10413765) 已經講過為什麼: 人設動的是語氣那一層, 動不到「它推成什麼任務」那一層. 今天補上證據.

EMNLP 2024 的研究 (Zheng et al.) 做得很徹底: 整理 **162 種角色** (涵蓋 6 種人際關係、8 個專業領域), 在 **4 個模型家族**上測 **2,410 道事實題**. 結論是: **加角色並沒有讓表現變好.**

> 注意範圍, 這測的是**客觀任務** (有標準答案的). 當「換一種說法」本身就是你要的東西時, 人設仍然有用. 這時它是在**指定任務**, 不是裝飾語氣 (這個區別 [Day 05](https://ithelp.ithome.com.tw/articles/10413765) 拆得更細).

> (jason3e7) 誠實補一句: 我自己四月寫的筆記裡, 好 prompt 的結構第一行就是「角色設定」.
...
把三段擺在一起, 規律很清楚, 而且全部能從 [Day 01](https://ithelp.ithome.com.tw/articles/10411345) 推出來:
