---
title: "AI 101 - 鐵人賽 Day 12: 為了方便人類驗證而生的兩個 skill"
tags: [ai, 鐵人賽, ironman, skill, mindmap, 驗證, 草稿]
created: 2026-09-26
status: draft
---
...
> **TL;DR (EN):** A wall of prose is expensive to verify; a mind map is scannable. Two skills change the shape of AI output so a human can check it fast. condense-mindmap (convergent) turns a big pile into a map where every node must trace back to the source - so you verify the map against the source and spot what is missing or over-claimed. expand-mindmap (divergent) grows a seed into a researched map and marks every speculative node with `(?)` - which is literally a to-do list of what to go check. Same output contract, opposite faithfulness stance.
...
## 為什麼心智圖幫驗證 — Why a Map Is Easier to Check

一大段文字很難驗. 它是線性的, 你得從頭讀到尾才知道它漏了什麼、哪裡誇大. 而**心智圖是結構化的**: 幾個主要分支一眼看完, 缺了哪一塊、哪一塊硬撐, 掃過去就發現.

這正好打 Day 11 那個瓶頸. Day 11 說「人的驗證判斷是 bottleneck」, 而瓶頸的一大來源是**產出的形狀不好驗**. 換個形狀, 驗證成本就掉下來.
...
**輸入一堆資料 (筆記、逐字稿、多份文件), 輸出一張濃縮的心智圖.** 這是收斂, 忠實度優先.

它幫驗證的關鍵, 藏在一條硬規則裡:

> **每個節點都必須能追回原始資料的哪一段, 加不出處的就是幻覺.**

所以這張圖天生可驗: 你拿圖上任一節點, 回原文找對應那段. 對得上 → 沒失真; 對不上 → 抓到它加料. 你不用重讀全文, 只要抽查幾個節點.

而且**缺漏也看得出來**: 圖上少了一整個你知道原文有講的主題, 那就是它漏了.

**自己試:** 拿一份長會議記錄丟給它, 收到心智圖後, 挑最重要的三個節點回原文對. 你會在 30 秒內知道這份濃縮能不能信 - 比重讀十頁快得多.

---

## expand-mindmap: 驗「哪裡要查」— A Checklist of What to Verify

**輸入一點種子 (一個主題、一個問題), 研究之後放大成一張圖.** 這是發散, 可以生, 但有一條為驗證而設的規則:

> **推測的節點一律標 `(?)`, 沒標的就代表它保證查證過.**

這個標記本身**就是一張驗證待辦清單**. 圖上標了 `(?)` 的, 就是它自己承認「這條我沒把握」- 你的驗證力氣直接往那些節點集中, 不用每條都查.
