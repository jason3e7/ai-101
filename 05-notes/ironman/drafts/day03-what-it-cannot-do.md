...
兩次都錯，錯得還不一樣，而且兩次都沒有說「我不確定」。但同一題交給 Fable 5.1，它答對了。所以這不是「LLM 會不會反轉詛咒」的問題，是哪個模型對哪個事實會的問題。
...
**我自己踩到的版本比這嚴重得多。** 在 HTB 靶機上跑 Claude Code 時，我開頭就交代「不要對 SSH 做遠端暴力破解」。前面幾輪它守得好好的；跑了一段時間之後，它還是開始暴力破解 SSH。
...
錯在中段 - 前 8 位對、後 3 位也對，壞在中間那幾位。片段是對的，組起來的時候接錯了，正是「子圖比對」會出的錯。

更有意思的是：同一個 Haiku，面對**看起來更難**的 10 位數乘法，反而誠實拒絕了 -「我無法在不使用計算工具的情況下精確計算這麼大的數字。」

難的那題說做不到，簡單的那題自信答錯。它對自己能力的認知，也是鋸齒狀的。
...
**我測到的（Haiku 4.5，2026-09-17，工具全部關掉）：** N=9 這題，它先輸出了一段**根本沒有執行過**的工具呼叫 -
