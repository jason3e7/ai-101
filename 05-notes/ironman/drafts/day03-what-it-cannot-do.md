...
> [!TIP]
> 這條直接決定了 Day 19 的主題：**驗證必須來自外部**——測試、編譯器、另一個獨立的模型，或者你自己。叫它「再檢查一次」，多數時候只是讓它把同一個錯誤講得更有信心。
...
這一條撐起了後面兩整段的內容：因為邊界看不見，所以要養成驗證習慣（Day 18–20）；因為新手掉最多，所以**用 AI 的人自己有多專業，決定了他能拿到多少收益**（Day 21–23）。
...
| 極限 | 類型 | 你該做的 |
|---|---|---|
| 不會自己發現自己錯 | 結構性 | 驗證一定要來自外部 |
| 反過來問就不會 | 結構性 | 換方向問，當成廉價的驗證 |
| 不會從對話中學會 | 結構性 | 用 `CLAUDE.md`、memory、RAG 補 |
| 精確計算不可靠 | 結構性 | 叫它寫程式算，別叫它心算 |
| 上下文中段會掉 | 暫時 | 重要的放頭尾 |
| 複雜度過線就崩 | 暫時 | 把任務拆小，並自己實測邊界 |
| 鋸齒狀前沿 | 看不見 | 預設它隨時可能站在錯的那一邊 |
