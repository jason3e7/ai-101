...
```text
LLM 在做的事 ＝ 猜下一個字（然後重複幾千次）
│
├── 這件事 1948 年就有了
│   └── Shannon 手翻書統計「哪個字後面常接哪個字」
│       今天只是把「看多長、統計多細、吃多少資料」推到極端
│
├── 兩個直接推論
│   ├── 為什麼講得順？ → 「順」就是它被訓練的目標本身
│   └── 為什麼自信地錯？→ 機率上「像樣的答案」比「我不知道」高分
│
├── 「沒學過的就不會」——對了一半
│   ├── 對的：知識截止日是硬邊界 → 這就是 RAG 存在的理由
│   └── 要修：它會的是「重組與內插」，不是背誦
│       ├── Othello-GPT：只餵棋步，內部卻長出棋盤狀態
│       └── Claude 寫詩：動筆前就先選好了韻腳
│
└── 所以後面 29 天，全都從這裡長出來
    ├── 會流暢地錯 → 要驗證（Day 19–21）
    ├── 機率由「全部脈絡」決定 → 餵什麼比怎麼問重要（Day 12–13）
    ├── 沒有「該停了」的概念 → 要給可驗證的終點（Day 16–18）
    └── 擅長重組、不擅無中生有 → 可以拿它拓展想像（Day 22–23）
```
