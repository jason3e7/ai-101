---
title: AI 101 - 鐵人賽 Day 01：LLM 的原理只是在猜下一個字
tags: [ai, 鐵人賽, ironman, llm, 原理, 統計, 草稿]
created: 2026-09-02
status: draft
---

# Day 01｜LLM 的原理只是在猜下一個字，這決定了所有心法
...
想像你只看得到一長串棋譜——「黑下這裡、白下那裡」——從頭到尾沒看過棋盤，也沒人告訴你規則。你唯一的任務是猜下一手會下在哪。

2022 年有一組研究者就是這樣訓練一個小模型的（用的是黑白棋）。訓練完把模型剖開來看，發現**它自己在腦中畫出了一張棋盤**：哪一格是黑、哪一格是白、哪一格還空著，都對得上。

更關鍵的是下一步。研究者直接動手改它腦中那張棋盤，把某一格的黑子換成白子。結果模型接下來猜的棋步，就照著被改過的盤面走。

**它不是在背棋譜，它真的建了一個「這盤棋現在長什麼樣」的模型。** 而它學到這件事，靠的只是一直猜下一手。

**二、Anthropic 掃描 Claude 的腦，看它回答時哪裡在動。**
...
這兩個實驗合起來，剛好回答了 2021 年一個很有名的批評。

那篇文章把語言模型比喻成**「隨機鸚鵡」**——只會把看過的話重新吐出來，自己什麼都不懂。今天回頭看，這個比喻要拆成兩半：它對偏見與過度炒作的批評，現在仍然成立；但「模型內部什麼都沒有、只是表面統計」這個說法，已經被上面兩個實驗推翻了。

不過反方向的說法也別照單全收。2023 年另一篇研究指出：很多「模型一變大，某個能力就突然冒出來」的驚人曲線，其實是**計分方式造成的錯覺**——原本用「整題全對才給分」，換成「對幾成給幾分」，那條陡然拉起的線就變回一條緩坡。

**兩邊都別信滿。** 這件事本身，就是後面會反覆用到的心法。
...
| 因為它…… | 所以…… | 在哪幾天 |
|---|---|---|
| 目標是「看起來合理」，不是「正確」 | 你必須有一套自己的驗證方法 | Day 19–21 |
| 下一個字的機率由前面**全部**內容決定 | 餵什麼，比怎麼問更重要 | Day 12–13 |
| 沒有內建「該停了」的概念，只會一直往下接 | 要給它可驗證的終點與護欄 | Day 16–18 |
| 擅長重組，不擅長無中生有 | 能帶你跨出自己的舒適圈，但跨不出人類知識的邊界 | Day 22–23 |
| 模糊的問題只會得到模糊分布下的抽樣 | 提問本身就要能被檢查 | Day 08–11 |
| 產出會留下可偵測的統計痕跡 | 標記與辨識做得到，但都不算證據 | Day 25 |
