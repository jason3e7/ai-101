...
## 兩個共同的設計問題 — Open Questions

留給「變成 skill」時要決定的：

- **深度上限**：心智圖要幾層？建議預設 3 層，太深就不是心智圖是大綱了。
- **要不要包 markmap 指令**：skill 只吐 markdown，還是順便給一句「貼到 markmap.js.org 看圖」？傾向只吐 markdown，渲染交給使用者的工具。
- **濃縮的輸入上限**：資料太多會塞爆 context。可能要先分段各自濃縮再合併（map-reduce），這點可參考 [六種能力執行手冊](../02-advanced/capabilities-playbook.md)摘要那節的長文處理。
- **放大要不要動用 web 研究**：如果種子是冷知識，沒有 web 就只能靠模型內部知識，品質會抖。skill 描述裡要講清楚它假設有沒有搜尋工具。
