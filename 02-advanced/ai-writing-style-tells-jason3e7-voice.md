---
title: AI 101 - AI 的文風與語氣（jason3e7 手筆改寫版）
tags: [ai, 文風, 語氣, em-dash, rlhf, 辨識, 進階, 文風改寫]
created: 2026-09-20
---
...
先講你觀察到的那個. 破折號 (em dash, 英文 `—`, 中文全形 `——`) 是 AI 文風習慣裡**最頑固、最好辨識**的一個. 它頑固到有一篇 2026 年的研究專門在講它, 標題就叫〈The Last Fingerprint〉(最後一個指紋).

成因有兩層, 兩層都不是「模型的選擇」:

**一、Markdown 洩漏進了散文.** 模型的訓練資料塞滿了 markdown, 標題、粗體、清單、破折號都是. 這些結構標記在被要求「寫成純散文」時, 大部分能被壓掉 (標題、清單都會消失), **唯獨破折號壓不掉**. 它同時是「合法的標點」又是「結構標記」, 你沒辦法在禁止它當結構符號的同時, 還允許它當正常標點. 它是那套結構習慣**最小的、活下來的那個單位**.

**二、RLHF 放大了它.** 破折號在高品質的編輯過的文本 (《紐約客》《大西洋月刊》那類) 裡出現得特別多. 模型從這些資料學到「好文章長這樣」. RLHF 階段的人類評分者多半是習慣結構化寫作的技術背景, 又傾向給這種「看起來工整」的文字更高分. 一個正回饋迴圈就這樣把它放大了.

> [!IMPORTANT]
> 關鍵證據: **這件事不是 LLM 的必然, 是特定訓練選擇的結果.** 同一篇研究測了十二個模型, 破折號密度天差地遠, 有些模型 (例如 Llama) 幾乎是零. 所以「AI 都這樣」是錯的觀察, 應該說「**某些訓練管線**這樣」.
...
> [!WARNING]
> **單獨一個都不算證據.** 破折號用了幾個世紀, 三段式是正常的修辭手法. 真正像 AI 的是**成群出現**, 三段式、對立句、反問句擠在同一段, 整篇還沒有一個具體的、會讓作者擔責任的判斷. 單一特徵抓不了人, 密度和缺乏擔當才是訊號.
...
> [!TIP]
> 最省事的做法: 把這幾條寫進 `CLAUDE.md` 或你的自訂風格 (style) 設定, 它每一輪都會重新套用, 不必每次重講. 這個 repo 自己就這樣做, 專案規範裡明訂「引用名言用一般 `>`, 破折號一律換成 ` - `」, 所以你在原版筆記裡看到的是 ` - ` 不是 `——`. (本篇再往前一步, 連 ` - ` 也拿掉, 全部改成句號或逗號.)
...
> [!WARNING]
> **一個反直覺的坑: `effort` 參數不能拿來縮短輸出.** 很多人以為把 effort 調低就會變短. 官方明講, 在 Claude Opus 5 上, effort 控制的是「想多久」, **不是回答的長度**. 調低 effort 不會可靠地讓回答變短. 要短, 只能用上面那些「明講長度」的方式. effort 管的是思考深度, 跟篇幅是兩件事.
...
壓文風習慣壓過頭, 文章會變得**乾巴巴、句子長度一致、不敢下判斷**, 那也是一種「AI 味」, 只是換了個方向. 目標不是消滅所有結構, 是**讓結構服務內容**. 留幾個破折號、留一個漂亮的三段式都沒問題. 問題永遠是**成群的、空洞的**那種.
...
有, 而且是同一條原理. 文風習慣壓不掉、能力選不準, 都來自「模糊指令等於模糊結果」. 解法也一樣: **具體、可檢查、給替代.**
